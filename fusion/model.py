...
class SubNet(nn.Module):
    ...
    def __init__(self, in_size, hidden_size, out_size, dropout, TT_SUBNET=False, shape=None, max_rank=20, tensor_type = 'TTM', device=None, dtype=None):
        ...
        if self.TT_SUBNET is True:
          ...
          #                                     prior_type='log_uniform', 
          #                                     eta=None, device=device, dtype=dtype)
          self.linear_1 = TensorizedLinear_module(in_size, hidden_size, bias=None, shape=self.shape_1, tensor_type=tensor_type, max_rank=max_rank,
                          prior_type=self.prior_type, eta=self.eta, device=device, dtype=dtype)
          self.linear_2 = TensorizedLinear_module(hidden_size, hidden_size, bias=None, shape=self.shape_2, tensor_type=tensor_type, max_rank=max_rank,
                          prior_type=self.prior_type, eta=self.eta, device=device, dtype=dtype)
          self.linear_3 = TensorizedLinear_module(hidden_size, out_size, bias=None, shape=self.shape_3, tensor_type=tensor_type, max_rank=max_rank,
                          prior_type=self.prior_type, eta=self.eta, device=device, dtype=dtype)
        else:  
