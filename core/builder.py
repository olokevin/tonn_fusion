...
def make_model(device: Device, random_state: int = None, configs=configs) -> nn.Module:
    if "mixer" in configs.model.name.lower():
        if configs.tensorize_config.en is True:
            tensorize_config=configs.tensorize_config
        else:
            tensorize_config=None
        model = eval(configs.model.name)(
            ...
            device=device,
            tensorize_config=tensorize_config,
        ).to(device)
        model.reset_parameters(random_state)
    elif "mlp" in configs.model.name.lower():
        model = eval(configs.model.name)(
        ...
        model.reset_parameters(random_state)
    elif "ttm" in configs.model.name.lower():
        model = SparseBP_MZI_TTM_MLP(
            n_feat=configs.dataset.img_height * configs.dataset.img_width,
            n_class=configs.dataset.n_class,
            hidden_list=configs.model.hidden_list,
            block_list=configs.model.block_list,

            in_shape=configs.model.in_shape,
            hidden_shape_list=configs.model.hidden_shape_list,
            out_shape=configs.model.out_shape,
            max_rank_list=configs.model.max_rank_list,
        ...
        model.reset_parameters(random_state)
    elif "lmf" in configs.model.name.lower():
        model = SparseBP_MZI_LMF(
            ...
            use_LSTM = configs.model.use_LSTM, 
