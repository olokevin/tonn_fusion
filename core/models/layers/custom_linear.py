...
class MZIBlockLinear(torch.nn.Module):
    ...
    ###### TTM init
    def ttm_factor_xavier_init(
            self,
            tensor: Tensor, 
            std_factors: float = 1.0
        ) -> Parameter:
        # tensor: Parameter
        # std_factors: 
        if not isinstance(tensor, Tensor):
            raise TypeError(
                f'Expecting the given tensor as a Tensor, got type={type(tensor)} instead.')
        
        with torch.no_grad():
            return tensor.normal_(0, std_factors)
    
    def reset_parameters_ttm(self, std_factors: float) -> None:
        if self.mode == "weight":
            # init.kaiming_normal_(self.weight.data)
            # return or not, the provided tensor is initialized
            self.ttm_factor_xavier_init(self.weight.data, std_factors)
        elif self.mode == "usv":
            W = self.ttm_factor_xavier_init(
                torch.empty(
                    self.grid_dim_y,
                    self.grid_dim_x,
                    self.miniblock,
    ...
    def gen_mixedtraining_mask(
        ...
        if self.mode == "weight":
            ...
            out = {
                ...
                "S": self.S.data > percentile(self.S.data, sparsity * 100),
            ...
            if prefer_small == False or self.phase_U.size(0) == self.phase_V.size(0):
                ...
                mask_S = torch.zeros_like(self.phase_S.data).bernoulli_(p=1 - sparsity)
                ...
                if total_nonzero <= self.phase_U.numel():
                    ...
                    mask_S = torch.zeros_like(self.phase_S, dtype=torch.bool)
                    # mask_S = torch.ones_like(self.S.data, dtype=torch.bool)
                else:
                    indices = (
                        torch.from_numpy(
    ...
    def get_2d_weight(self) -> Tensor:
        if self.weight is None:
            weight = self.build_weight()  # [p, q, k, k] or u, s, v
        else:
            weight = self.weight
        
        return weight.permute([0, 2, 1, 3]).contiguous().view(self.out_channel_pad, self.in_channel_pad)[:self.out_channel, :self.in_channel]
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.in_channel_pad > self.in_channel:
            if self.x_zero_pad is None or self.x_zero_pad.size(0) != x.size(0):
                ...
                # self.x_zero_pad = torch.zeros(
                #     (x.shape[:-1])+((self.in_channel_pad - self.in_channel),), device=x.device, dtype=x.dtype
                # )
        ...
        if self.bias is not None:
            if self.out_channel_pad > self.out_channel:
                bias = torch.cat([self.bias, torch.zeros(self.out_channel_pad - self.out_channel, device=self.bias.device)])
            else:
                bias = self.bias
        else:
            bias = None
